...
if ($d['t']['display_webp_images'] == 1) {
    ...
    if ($d['t']['lazy_load_category_items'] == 1) {
        ...
        echo '<picture>';
        //echo '<source type="image/webp" data-src="'. $srcWebP.'" alt="' . $altValue . '" class="' . $class . '" style="width:100%;height:auto;display:block;" data-srcset="' . $srcSetWebP . '" data-image="' . $dataImgWebP . '" />';
       // echo '<source type="image/webp" data-src="'. $srcWebP.'" alt="' . $altValue . '"  data-srcset="' . $srcSetWebP . '" data-image="' . $dataImgWebP . '" />';// TEST
        ...
        echo '<img src="'.$srcPlaceHolder.'" data-src="'. $src.'" alt="'.$altValue.'" class="'.$class.'" style=\"width:100%;height:auto;display:block;\" data-image="'. $dataImg.'" />';
        echo '</picture>';

    } else {

        echo '<picture>';
        //echo '<source type="image/webp" alt="' . $altValue . '" class="' . $class . '" style="width:100%;height:auto;display:block;" srcset="' . $srcSetWebP . '" data-image="' . $dataImg . '" />';
        // class needed because of chaning attributes - changing attributes changes images
        // alt="' . $altValue . '"
        echo '<source type="image/webp" srcset="' . $srcSetWebP . '" data-image="' . $dataImg . '" class="'.$classSource.'" />';// TEST
        echo '<img src="' . $srcImg . '" alt="' . $altValue . '" class="' . $class . '" style="width:100%;height:auto;display:block;" data-image="' . $dataImg . '"'.$attributeLazyLoad.' />';
        echo '</picture>';

        // Switch
        if (isset($d['image']['second']->rel_webp) && $d['image']['second']->rel_webp != '') {
            $switchImg      = Uri::base(true).'/'.$d['image']['second']->rel_webp; // switch
            echo '<span class="phIRBox"><img src="'. $switchImg.'" alt="'.$altValue.'" class="'.$classSwitch.'" style=\"width:100%;height:auto;display:block;\" /></span>';
        }
    ...
    if ($d['t']['lazy_load_category_items'] == 1) {
        ...
        echo '<img src="'. $src.'" alt="'.$altValue.'" class="'.$class.'" style=\"width:100%;height:auto;display:block;\" data-image="'. $dataImg.'"'.$attributeLazyLoad.' />';
...
data-src="'. $src.'" - is in webp source for
    //echo '<img src="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 640 428\'%3E%3C/svg%3E" data-src="'. Uri::base(true).'/'.$d['image']->rel.'" alt="'.$altValue.'" class="ph-lazy img-responsive ph-image '. $d['image']['phil'].' phjProductImage'.$productIdName.'" style=\"width:100%;height:auto;display:block;\" data-image="'. Uri::base(true).'/'.$d['default_image']->rel.'" />';
...
echo '<source type="image/webp" alt="'.$altValue.'"  data-src="'. Uri::base(true).'/'.$d['image']->rel_webp.'" alt="'.$altValue.'"  data-srcset="'. Uri::base(true).'/'.$d['image']->rel_webp.'" alt="'.$altValue.'"  class="ph-lazy img-responsive ph-image '. $d['image']['phil'].' phjProductImage'.$productIdName.'" style=\"width:100%;height:auto;display:block;\" data-image="'. Uri::base(true).'/'.$d['default_image']->rel.'" />';
